...
function OpenCloakroomMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'cloakroom',
		{
			...
			align    = 'top-left',
			elements = {
				{label = _U('minerador_clothes_civil'), value = 'citizen_wear'},
				{label = _U('minerador_clothes_minerador'), value = 'mineradora_wear'},
			},
		},
		function(data, menu)
			...
			if data.current.value == 'mineradora_wear' then
				ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
					if skin.sex == 0 then
			...
			CurrentAction     = 'mineradora_actions_menu'
...
function OpenVigneActionsMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'mineradora_actions',
		{
			title    = 'Mineradora - Vale do Rio Doce',
			align    = 'top-left',
			elements = elements
		},
		...
		function(data, menu)
			...
			if data.current.value == 'boss_actions' then
				TriggerEvent('esx_society:openBossMenu', 'mineradora', function(data, menu)
					menu.close()
				end)
			end
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open(
				...
				function(data, menu)
					...
					TriggerServerEvent('esx_society:removeVehicleFromGarage', 'mineradora', vehicleProps)
...
function OpenMobileVigneActionsMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'mobile_mineradora_actions',
		{
			title    = 'mineradora',
			align    = 'top-left',
			elements = {
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								Citizen.CreateThread(function()
									...
									TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_mineradora', 'Mineradora', amount)
								end)
							end
						end
					end,
...
function OpenGetStocksMenu()
	...
	ESX.TriggerServerCallback('esx_mineradorjob:getStockItems', function(items)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_mineradorjob:getStockItem', itemName, count)
						end
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('esx_mineradorjob:getPlayerInventory', function(inventory)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_mineradorjob:putStockItems', itemName, count)
						end
...
AddEventHandler('esx_mineradorjob:hasEnteredMarker', function(zone)
	if zone == 'Mina' and PlayerData.job ~= nil and PlayerData.job.name == 'mineradora'  then
		CurrentAction     = 'pedra_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end
		
	if zone == 'TratamentoFerro' and PlayerData.job ~= nil and PlayerData.job.name == 'mineradora'  then
		CurrentAction     = 'ferro_traitement'
		CurrentActionMsg  = _U('press_traitementf')
		CurrentActionData = {zone= zone}
	end		
		
	if zone == 'TratamentoAluminio' and PlayerData.job ~= nil and PlayerData.job.name == 'mineradora'  then
		CurrentAction     = 'aluminio_traitement'
		CurrentActionMsg  = _U('press_traitementa')
		CurrentActionData = {zone = zone}
	end
	
	if zone == 'TratamentoCobre' and PlayerData.job ~= nil and PlayerData.job.name == 'mineradora'  then
		CurrentAction     = 'cobre_traitement'
		CurrentActionMsg  = _U('press_traitementc')
		CurrentActionData = {zone= zone}
	end		
		
	if zone == 'TratamentoPrata' and PlayerData.job ~= nil and PlayerData.job.name == 'mineradora'  then
		CurrentAction     = 'prata_traitement'
		CurrentActionMsg  = _U('press_traitementp')
		CurrentActionData = {zone = zone}
	end
	
	if zone == 'TratamentoOuro' and PlayerData.job ~= nil and PlayerData.job.name == 'mineradora'  then
		CurrentAction     = 'ouro_traitement'
		CurrentActionMsg  = _U('press_traitemento')
		CurrentActionData = {zone = zone}
	end
	
	if zone == 'Venda' and PlayerData.job ~= nil and PlayerData.job.name == 'mineradora'  then
		CurrentAction     = 'farm_resell'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	end

	if zone == 'MineradoraAcoes' and PlayerData.job ~= nil and PlayerData.job.name == 'mineradora' then
		CurrentAction     = 'mineradora_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end
  
	if zone == 'VehicleSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'mineradora' then
		CurrentAction     = 'vehicle_spawner_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	end
		
	if zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'mineradora' then
...
AddEventHandler('esx_mineradorjob:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if (zone == 'Mina') and PlayerData.job ~= nil and PlayerData.job.name == 'mineradora' then
		TriggerServerEvent('esx_mineradorjob:stopHarvest')
	end  
	if (zone == 'TratamentoFerro' or zone == 'TratamentoAluminio' or zone == 'TratamentoCobre' or zone == 'TratamentoPrata' or zone == 'TratamentoOuro' ) and PlayerData.job ~= nil and PlayerData.job.name == 'mineradora' then
		TriggerServerEvent('esx_mineradorjob:stopTransform')
	end
	if (zone == 'Venda') and PlayerData.job ~= nil and PlayerData.job.name == 'mineradora' then
		TriggerServerEvent('esx_mineradorjob:stopSell')
	end
	CurrentAction = nil
end)
...
function blips()
	if publicBlip == false then
		local blip = AddBlipForCoord(Config.Zones.MineradoraAcoes.Pos.x, Config.Zones.MineradoraAcoes.Pos.y, Config.Zones.MineradoraAcoes.Pos.z)
		...
		AddTextComponentString("Mineradora - Vale do Rio Doce")
		EndTextCommandSetBlipName(blip)
		publicBlip = true
	end
	
    if PlayerData.job ~= nil and PlayerData.job.name == 'mineradora' then
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'mineradora' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_mineradorjob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_mineradorjob:hasExitedMarker', LastZone)
			end
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'mineradora' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'pedra_harvest' then
					TriggerServerEvent('esx_mineradorjob:startHarvest', CurrentActionData.zone)
				end
				if CurrentAction == 'ferro_traitement' then
					TriggerServerEvent('esx_mineradorjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'aluminio_traitement' then
					TriggerServerEvent('esx_mineradorjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'cobre_traitement' then
					TriggerServerEvent('esx_mineradorjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'prata_traitement' then
					TriggerServerEvent('esx_mineradorjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'ouro_traitement' then
					TriggerServerEvent('esx_mineradorjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'farm_resell' then
					TriggerServerEvent('esx_mineradorjob:startSell', CurrentActionData.zone)
				end
				
				if CurrentAction == 'mineradora_actions_menu' then
				...
				if CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicleInGarage', 'mineradora', vehicleProps)
					end
		...
		if IsControlPressed(0,  Keys['F6']) and Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == 'mineradora' and (GetGameTimer() - GUI.Time) > 150 then
